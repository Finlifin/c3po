...
- **上传接口**
  - `POST http://10.70.141.134:5000/api/v1/images/upload`
  ...
  - 响应：
    ...
    ```
    - `url` 为文件名（含扩展名）；前端可拼接成 `http://10.70.141.134:5000/api/v1/images/{url}` 访问。
    - 若文件已存在，会复用原文件名，保证幂等。
  - 失败示例（缺少文件）：
    ```json
    {
...
- **下载接口**
  - `GET http://10.70.141.134:5000/api/v1/images/{filename}`
