...
const moduleDisplayOrder = ref<number>(1)
...
const createModule = async () => {
  ...
  if (!moduleDisplayOrder.value || moduleDisplayOrder.value < 1) {
    ElMessage.warning('请输入有效的显示顺序（整数 ≥ 1）')
    return
  }
  ...
  try {
    ...
    await teacherApi.createModule(courseId, {
      title: moduleTitle.value,
      displayOrder: moduleDisplayOrder.value,
    ...
    // 重置表单
    moduleTitle.value = ''
    moduleDisplayOrder.value = 1

  } catch (err: any) {
    console.error('创建章节失败:', err)
    ElMessage.error(err.response?.data?.message || '创建章节失败')
  } finally {
  ...
  <div class="course-resources-container">
    ...
    <el-card class="course-info-card" v-if="course">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
        ...
        <el-button type="primary" size="small" @click="showCreateModuleModal = true">
          <el-icon class="el-icon--left"><Plus /></el-icon>创建章节
        </el-button>
      </div>
    </el-card>
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="showCreateModuleModal = false">取消</el-button>
