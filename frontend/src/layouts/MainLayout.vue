...
const user = computed(() => {
  if (isStudent.value) {
    return studentAuthStore.user
  } else {
    return authStore.user
  }
})
const isStudent = computed(() => {
  // Check path first as fallback, then check role from appropriate store
  if (route.path.startsWith('/student')) return true
  
  if (studentAuthStore.isAuthenticated) {
    return studentAuthStore.role === 'ROLE_STUDENT'
  } else {
    return authStore.role === 'ROLE_STUDENT'
  }
})
...
const getAvatarUrl = (avatarUrl: string | null | undefined) => {
  ...
  const ossBaseUrl = import.meta.env.VITE_OSS_BASE_URL || 'http://10.70.141.134:5000'
...
const pageTitle = computed(() => {
  ...
  if (path.includes('ai-chat')) return 'AI学习助手'
  return '首页'
})
...
const menuItems = computed(() => {
  if (isStudent.value) {
    return [
      ...
      { label: 'AI学习助手', path: '/student/ai-chat', icon: '🤖', badge: undefined },
